...
cout<<"l.translation \n 2. shearing In 3.reflection n 4.rotation n S.scaling n 6.exit"<<endl;
cin>>ch;
switch(ch){
case 1:
{
...
float rx;
rx=(theta*3.14)/180;
float T[3][3]={{cos(rx),sin(rx),0},{-sin(rx),cos(rx),0},{
                                    0, 0, 1}};
for(i-0;i<2;it+){
                                for (j - 0j < 3j ++)
                                    for (k = 0; k < 3; k++)
PPliJÜ+=PÜIK]*T[KIÜ);
line(pp[0][0). pp[O][1]. pp[1][0]. pp[1][1):
system("pause");
break;
case 5:
int Sx,Sy:;
...
cin>>Sy;
int T[3][3]={{Sx,0,1}.{0,Sy. I }.{0,0, 1)}:
for(i-0;i<2;i++)
                                            {
                                                for (ý - 0; j < 3; j++)
                                                    for (k - 0; k < 3; kt +)
PPliJi]+=PÜIK]"T[KIL0J: }
line(pp[0][0]. pp[O][ 1]. pp[1J[O], Pp[ID:
system("pause");
break:
                                            }}
return 0;}